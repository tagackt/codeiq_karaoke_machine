...
class KaraokeMachine
  ...
  def transpose(amount)
    ...
    melody_num = replaceKeyToNum(@melody)

    #キー移動
    #配列で取り出し、キーを変更する
    splited_melody_nums = melody_num.split(",").collect {|num| num == " " || num == "|" ? num : num.to_i + amount}
    ...
    return_melody = replaceNumToKey(splited_melody_nums)

    return_melody
   end
   
   private
   def replaceKeyToNum melody
   ...
   private
   def replaceNumToKey splited_melody_nums
    #数値をキーに置換する
    replaced_melody = splited_melody_nums.join(",").gsub(/1[0-1]|[0-9]| |\|/,
      "0" => "C",
      "1" => "C#",
      "2" => "D",
      "3" => "D#",
